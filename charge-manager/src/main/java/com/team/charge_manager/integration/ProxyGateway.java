 ...
 * Optional: SOAP/RPC (required by the professor diagram) when
 * proxy.soap.enabled=true
...
public class ProxyGateway {
    ...
    public ProxyCustomerResponse createCustomer(ProxyCustomerRequest request) {
        if (soapEnabled && soapFactory.isConfigured()) {
            System.out.println("MANAGER-INTEGRATION -> Usando comunicação SOAP-RPC para criar cliente");
        ...
        System.out.println("MANAGER-INTEGRATION -> Usando comunicação REST para criar cliente");
    ...
    public ProxyChargeResponse createCharge(ProxyChargeRequest request) {
        if (soapEnabled && soapFactory.isConfigured()) {
            System.out.println("MANAGER-INTEGRATION -> Usando comunicação SOAP-RPC para criar cobrança");
        ...
        System.out.println("MANAGER-INTEGRATION -> Usando comunicação REST para criar cobrança");
    ...
    public void cancelCharge(String asaasPaymentId) {
        if (soapEnabled && soapFactory.isConfigured()) {
            System.out.println("MANAGER-INTEGRATION -> Usando comunicação SOAP-RPC para cancelar cobrança");
            soapFactory.paymentClient().cancelCharge(asaasPaymentId);
            return;
        }
        System.out.println("MANAGER-INTEGRATION -> Usando comunicação REST para cancelar cobrança");
